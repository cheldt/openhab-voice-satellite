...
def test_openwakeword_never_skips_a_frame(detector_factory):
    """The `| None` skip contract exists for livekit and must not reach here.

    `_scores` may now return None, which suppresses the decision for that
    frame. openwakeword scores incrementally and always returns a dict, so
    this pins that the widened hook left its behaviour alone.
    """
    detector = detector_factory({"wake": [0.1] * 5}, model="wake")
    for _ in range(5):
        detector.process(FRAME)
