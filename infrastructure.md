...
| `piper_tts.py` | `PiperSpeaker`: per-language voices, chunk-level overlapped synthesis, RTF debug logging | Splits with `tts_chunks`, not `split_sentences`: the case the 400-char cap exists for is a "list all items" answer that is one giant comma-separated sentence, and punctuation alone hands piper a single `synthesize()` call whose whole output materializes before a byte can play — pipelining gone, silence in SPEAKING for the duration. `__init__` loads every configured voice up front — the RAM figure behind §3.1's `LazySpeaker`. Unknown language falls back to the default language's voice via a bare dict access that cannot `KeyError` only because a `default_language` without a voice is a load-time error (§3.1). Empty synthesis returns `rate=0`, which `play_pipelined` skips via `if len(pcm)` |
...
| `fallback.py` | `CloudEngineError`, `PartialSpeechError`, `FALLBACK_ERRORS`, `FallbackTranscriber`, `FallbackSpeaker`, `LazySpeaker` | `CancelledError` (barge-in) is not an `Exception` and passes through both wrappers untouched; `LazySpeaker` defers the multi-second local model load to the executor, memoized and shielded so a barge-in mid-load does not start a second one. `FALLBACK_ERRORS` is a closed set (HTTP, timeout, JSON decode, **body decode**); payload-decode failures inside the providers are wrapped in their own error types so they stay inside it, and `UnicodeDecodeError` is named explicitly because a response body is not ours to encode — aiohttp decodes strictly, so a TLS-terminating middlebox, an overridden plain-HTTP `base_url` or a provider edge answering in ISO-8859-1 raises it out of `resp.text()`. It is a `ValueError`, so it was the one malformed-payload shape that bypassed both wrappers |
...
| Component | Responsibility | Notable contract |
|---|---|---|
| `selftest.py` | `--check`: audio devices + real capture probe, wakeword (2.5 s of frames so the engine's context window fills, every configured head range-checked per frame, then one stage-2 inference over the filled ring), VAD, whisper, piper, cloud probes (gemini: model metadata per configured direction; deepgram: key check plus a silence `/v1/listen` and a one-word `/v1/speak` per voice), openHAB interpreters probe (proves the token and the voice subsystem, not just reachability). Each check imports lazily and reports independently; exit 1 on any failure | Every head whose scores a threshold is read against is checked, wake **and** stop — openwakeword passes raw model output through unclamped, so a stop model exported without its sigmoid used to pass here and then fire on nearly any speech during playback. Silence never crosses stage 1, so the verifier is invoked directly once at the end rather than left to the load-time shape checks: a graph that fails at run time, or a head emitting a logit, would otherwise pass a green `--check` and break the monitor loop at the first real wake. What `--check` still does **not** prove: playback (the output node is resolved but never opened — a dead sink passes clean; `--probe-mic`'s earcons are the playback test). The cloud probes are asymmetric in strength: gemini's is metadata-only (key, name, reachability — a model that cannot serve AUDIO or JSON mode still fails at the first utterance), deepgram's is a real per-direction round trip |
| `probe.py` | `--probe-mic`: 30 s field diagnostic on the same source/sink the app uses — per-second RMS/peak/stage-1 score plus wall and process CPU ms per frame against the `frame_ms` budget, earcons at t=8 s and t=18 s, capture written to `diagnose_capture.wav`. The module docstring is the interpretation guide (silent node, wrong node, clock mismatch, playback poisoning capture, stage-2 rejection, sleeping speaker) | The WAKE/STOP marks come from `process()`'s verdict — patience, the state-dependent threshold (`speaking=True` while the sink is audible) and the stage-2 verifier included — so a marked row is what the app would have acted on; the score columns stay stage 1, and a stage-1 trigger the verifier swallowed is marked `rejected by stage 2` with the verifier's score in its own column. The `stop_score` and `verifier` columns exist only when their models are configured (`score("stop")` without a stop model falls back to the wake trigger). Rows are paced on elapsed wall seconds, not seconds of audio, so under-delivery shows instead of stretching the run (plus a 0.8 × expected-fps warning); `verify_links` runs as a side task; capture accounting is read before `audio_io` closes the source; an earcon failure is reported, never raised, and so is a `diagnose_capture.wav` that cannot be written — it is CWD-relative, so a run from a directory the service user cannot write must not cost the summary the whole 30 s bought. Exit 1 on a stalled capture or unreachable PipeWire (same `deploy/install.md` hint as `--list-devices`), so the probe can gate scripts the way `--check` does |
| `bench.py` | Offline wakeword evaluation through `build_detector` and the real `EdgeTrigger`: `--score-wav` (per-file distribution + threshold × patience sweep over 15 thresholds × patience 1–3), `--compare MODEL` (two candidates on identical frames), `--positives/--negatives` (recall vs false accepts per hour — the promotion gate). Appends silence after each file so a deferred stage-2 verdict still flushes | `score()` is stage 1, so the sweep, the percentiles and every `best:` line describe stage 1 alone; each report also carries a `live` column/line — what `process()` returned, verifier and all — and `--compare` across a two-stage config refuses a sweep verdict (`_warn_on_asymmetry`) and points at the live lines. The flush is pinned to the `Stage2Config.delay_ms` ceiling (1000 ms) and feeds `live` only — never the sweep, the percentiles, or the frame count the false-accepts-per-hour figure divides by. The merged file list (`--score-wav` + `--positives` + `--negatives`) is deduped order-preserving, so a path reachable through two groups cannot shift the per-config columns. `_override` re-validates rather than `model_copy`ing — a `--model` override a hand-written config would be rejected for is rejected here too. Corpus arguments fail closed: a directory that exists but holds no WAVs is named and exits 2 rather than falling into the generic needs-both-flags message at exit 0, and a file that cannot be scored at all (wrong sample rate, corrupt container) is reported per row, dropped from every group so the gate's per-hour denominator stays honest, and turned into exit 2 after the full report — one 44.1 kHz recording no longer aborts the run mid-table |

**The sweep-vs-`live` asymmetry is bench's central design decision.** A config with a
stage-2 verifier runs stage 1 deliberately low (v6 two-stage sat at 0.10) *because* the
verifier is the real gate — so stage-1 numbers alone flatter nothing and rank nothing.
Every mode therefore reports both instruments and refuses the one comparison (`best:`
vs `best:` across two-stage configs) that would silently rank two detuned stage 1s.
The probe inherits the same discipline in the field: its marks are `process()` verdicts,
not threshold crossings, so what an operator sees on the device is what the app decides.
...
| `scripts/download_models.py` | Fetch openWakeWord models, Piper voices, whisper warmup. Piper URLs are pinned to an HF **commit revision**, not `main`, and each file's SHA-256 is verified in its `.part` before the rename — these `.onnx` files are parsed by onnxruntime inside the always-listening service, so a substituted model is native code execution. openWakeWord's shared feature models come down through its own downloader and stay unverified; that gap is documented in the function rather than hidden |
...
| `deploy/openhab-voice-satellite.service` | systemd **user** unit (PipeWire lives in the user session; needs `loginctl enable-linger`). Sets `HF_HOME`, `OMP_WAIT_POLICY=PASSIVE`, `OPENBLAS_NUM_THREADS=1`. `Restart=on-failure` pairs with the non-zero capture-death exit (§3.2). Carries a sandboxing block — the process parses network responses, runs native parsers on untrusted audio and optionally unpickles verifier files, so a memory-corruption bug there would otherwise own the whole user session. `MemoryDenyWriteExecute` stays off (onnxruntime/ctranslate2 map executable pages); `ReadWritePaths` is the install tree alone, which is why `HF_HOME` lives inside it. A user unit honors a version-dependent subset, so `systemd-analyze --user security` is part of the install |
| `deploy/install.md` | apt packages, a hash-pinned `uv export` install plus the hash-pinned openwakeword `--no-deps` line (so the Pi runs the same set the nightly audit scans, integrity-checked), `chmod 600 config.yaml` / `chmod 750` on the install dir, AEC config, 16 kHz graph clock pinning, powered-speaker preamble notes |
...
| `.github/workflows/security-scan.yml` | Nightly `pip-audit` over the locked dependency set **plus the out-of-lock production pins** — `openwakeword==0.6.0` is absent from project metadata by design and frozen forever by `wakeword_buffer.PATCHED_VERSION`, so without appending it the one dependency guaranteed to grow stale was the only one the scan could not see |
