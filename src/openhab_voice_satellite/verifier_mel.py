"""Mel-PCEN frontend for the stage-2 verifier, numpy/scipy only.

The verifier model (ultiwake, scripts/train_verifier.py) was trained on
features computed through librosa. This box deliberately has no librosa or
torch, so this module replicates that pipeline byte-for-byte from numpy +
scipy:
