...
# enough frames for every engine's context to fill: openwakeword needs 16
# embeddings behind 760 ms of mel, which is the longest of the three
...
def check_wakeword(config: Config) -> None:
    ...
    # one frame proves nothing: every engine here is muted until its context
    # window fills, so a model whose scores are not probabilities at all would
    # pass while still reporting its startup zero
